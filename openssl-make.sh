...
SOURCE_SHA1="238e001ea1fbf19ede43e36209c37c1a636bb51f"
BASE_VERSION="1.0.2"
VERSION="$(BASE_VERSION)m"
MIN_IOS_SDK="11.0"
DEST_FILE="openssl-${VERSION}.tar.gz"
SOURCE_URL="https://ftp.openssl.org/source/old/$BASE_VERSION/$DEST_FILE"
SDKVERSION=`xcrun --sdk iphoneos --show-sdk-version 2> /dev/null`
MIN_SDK_VERSION_FLAG="-miphoneos-version-min=$(MIN_IOS_SDK)"
BASEPATH="${PWD}"
CURRENTPATH="/tmp/openssl"
ARCHS="i386 x86_64 armv7 armv7s arm64"
DEVELOPER=`xcode-select -print-path`

curl --output "$DEST_FILE" -L "$SOURCE_URL"
echo "$SOURCE_SHA1  $DEST_FILE" > ./CHECKSUM

if [ !$(shasum --check CHECKSUM -s) ] ; then
  echo "Checksum match!"
else
  echo "Checksum failure for $SOURCE_URL"
  exit 1
fi

mv -v "$DEST_FILE" "file.tgz"
